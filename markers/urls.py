...
from .views import BookmarksApiView, TagsApiView, TagsApiDetail, BookmarksUpdateDestroyAPIView
from .custom_claims import TokenObtainPairView

urlpatterns = [
    path("bookmarks", BookmarksApiView.as_view(), name="bookmark-list"),
    path(
        "bookmarks/<int:pk>", BookmarksUpdateDestroyAPIView.as_view(), name="bookmark-update"
    ),
    re_path(
        "^bookmarks/(?P<key>.+)/$",
